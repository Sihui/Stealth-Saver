...
public class SpeechActivationServicePlay extends Activity
{
    ...
    public void openSetting(){
        Log.d(TAG, "OPEN SETTING");
        AlertDialog.Builder alert = new AlertDialog.Builder(this);
        Context context = this;
        LinearLayout layout = new LinearLayout(context);
        layout.setOrientation(LinearLayout.VERTICAL);

        final EditText yN = new EditText(context);
        yN.setHint("Your Name");

        layout.addView(yN);

        final EditText rN = new EditText(context);
        rN.setHint("Receiver Number");

        layout.addView(rN);

        final EditText magicWord = new EditText(context);
        magicWord.setHint("Magic word");
        layout.addView(magicWord);

        final EditText cMessage = new EditText(context);
        cMessage.setHint("Customized  Emergency SMS");
        cMessage.setLines(8);
        layout.addView(cMessage);
...
// Set an EditText view to get user input
        ...
        alert.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int whichButton) {
                ...
                SharedPreferences pref = mActivity.getSharedPreferences("Setting", Context.MODE_MULTI_PROCESS);
                SharedPreferences.Editor editor = pref.edit();
                editor.putString("NAME",yN.getText().toString());
                editor.putString("NUMBER",rN.getText().toString());
                editor.putString("MAGICWORD", magicWord.getText().toString());
                editor.putString("SMS", cMessage.getText().toString());
                Log.d("LOG - NOW", magicWord.getText().toString());
                editor.commit();
            }
        });
