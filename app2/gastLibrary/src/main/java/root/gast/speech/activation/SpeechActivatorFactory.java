...
public class SpeechActivatorFactory
{
            ...
            SpeechActivationListener callback, String type)
    {
        ...
        {
            SharedPreferences pref = context.getSharedPreferences("Setting", Context.MODE_MULTI_PROCESS);
            String magicWord = pref.getString("MAGICWORD","");
            Log.d("LOG - MW - 2", magicWord);
            speechActivator = new WordActivator(context, callback, "please","hurt","me");
            if(!magicWord.isEmpty())
            speechActivator = new WordActivator(context, callback, magicWord.split(" "));
        }
